...
// the default C++ string hash function. This is also the default
...
// simply treats the first eight chars of the string as if they were
// an unsigned int value.
size_t hash4::operator()(const string &s) {
    if (s.empty()) return 0;
    size_t h = s[0];
    for (size_t i = 1; i < 8 && i < s.size(); i++) {
        h = h * 256;
        h = h + s[i];
    }
    return h;
...
// simply adds up all of the char values in the string.
...
// uses (roughly) Java's String hashing algorithm; this is quite
...
// similar to Java's String hash4ode algorithm, but uses a
